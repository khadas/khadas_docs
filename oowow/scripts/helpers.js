...
hexo.extend.helper.register('redirect_link',function(title, header_text, path){
  if (title === header_text){
    ...
    if (
      startsWith(canonicalPath, 'oowow') ||
...
hexo.extend.helper.register('raw_link', function(path) {
  return 'https://github.com/khadas/khadas_docs/edit/master/oowow/source/' + path;
});
...
hexo.extend.helper.register('canonical_path_for_nav', function() {
  ...
  if (startsWith(path, '') || startsWith(path, 'vim1/') || startsWith(path, 'vim2/') || startsWith(path, 'vim3/') || startsWith(path, 'vim4/') || startsWith(path, 'edge/') || startsWith(path, 'hardware/') || startsWith(path, 'firmware/') || startsWith(path, 'tone1/') || startsWith(path, 'faq/')) {
    return path;
  }
  return '';
